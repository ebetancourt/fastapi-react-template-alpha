...
In that case, you will need to use a fake local domain (`dev.fullstack-template.com`) and make your computer think that the domain is is served by the custom IP (e.g. `192.168.99.150`).

If you used the default CORS enabled domains, `dev.fullstack-template.com` was configured to be allowed. If you want a custom one, you need to add it to the list in the variable `BACKEND_CORS_ORIGINS` in the `.env` file.

* Open your `hosts` file with administrative privileges using a text editor:
  * **Note for Windows**: If you are in Windows, open the main Windows menu, search for "notepad", right click on it, and select the option "open as Administrator" or similar. Then click the "File" menu, "Open file", go to the directory `c:\Windows\System32\Drivers\etc\`, select the option to show "All files" instead of only "Text (.txt) files", and open the `hosts` file.
  * **Note for Mac and Linux**: Your `hosts` file is probably located at `/etc/hosts`, you can edit it in a terminal running `sudo nano /etc/hosts`.

* Additional to the contents it might have, add a new line with the custom IP (e.g. `192.168.99.150`) a space character, and your fake local domain: `dev.fullstack-template.com`.

The new line might look like:

```
192.168.99.100    dev.fullstack-template.com
```
...
To configure it in your stack, follow the section **Change the development "domain"** below, using the domain `dev.fullstack-template.com`.

After performing those steps you should be able to open: http://dev.fullstack-template.com and it will be server by your stack in `localhost`.
...
```bash
STACK_NAME=stag-fullstack-template-com sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.fullstack-template-com.app-db-data == true
```
...
```bash
docker node update --label-add fullstack-template-com.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add stag-fullstack-template-com.app-db-data=true cat.example.com
```
...
* Set these environment variables:
  * `DOMAIN=fullstack-template.com`
  * `TRAEFIK_TAG=fullstack-template.com`
  * `STACK_NAME=fullstack-template-com`
  * `TAG=prod`
...
Frontend: https://fullstack-template.com

Backend: https://fullstack-template.com/api/

Automatic Interactive Docs (Swagger UI): https://fullstack-template.com/docs

Automatic Alternative Docs (ReDoc): https://fullstack-template.com/redoc

PGAdmin: https://pgadmin.fullstack-template.com

Flower: https://flower.fullstack-template.com

### Staging URLs

Staging URLs, from the branch `master`.

Frontend: https://stag.fullstack-template.com

Backend: https://stag.fullstack-template.com/api/

Automatic Interactive Docs (Swagger UI): https://stag.fullstack-template.com/docs

Automatic Alternative Docs (ReDoc): https://stag.fullstack-template.com/redoc

PGAdmin: https://pgadmin.stag.fullstack-template.com

Flower: https://flower.stag.fullstack-template.com
...
Frontend: http://dev.fullstack-template.com

Backend: http://dev.fullstack-template.com/api/

Automatic Interactive Docs (Swagger UI): https://dev.fullstack-template.com/docs

Automatic Alternative Docs (ReDoc): https://dev.fullstack-template.com/redoc

PGAdmin: http://dev.fullstack-template.com:5050

Flower: http://dev.fullstack-template.com:5555

Traefik UI: http://dev.fullstack-template.com:8090
